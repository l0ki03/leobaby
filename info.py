...
BOT_TOKEN = environ.get('BOT_TOKEN', "7259470022:AAExD9uDVT-hFUyBA35ZKkxoNKCBV-FdCAI")
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002041554852').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1001849039538')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '0')
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002011612381')
REQST_CHANNEL = int(reqst_channel) if reqst_channel and id_pattern.search(reqst_channel) else None
SUPPORT_CHAT_ID = int(support_chat_id) if support_chat_id and id_pattern.search(support_chat_id) else None
NO_RESULTS_MSG = bool(environ.get("NO_RESULTS_MSG", False))

# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://retihaf556:<wpCVabWTpQhFR1Nx>@cluster0.ctong.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/HMmedia_movie_group')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/HMmedia_Movie')
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/HMmedia_movie_group')
IS_TUTORIAL = bool(environ.get('IS_TUTORIAL', True))
MSG_ALRT = environ.get('MSG_ALRT', 'Wʜᴀᴛ Aʀᴇ Yᴏᴜ Lᴏᴏᴋɪɴɢ Aᴛ ?')
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1001990420435'))
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'HMmedia_movie_group')
